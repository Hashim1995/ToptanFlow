...
function isBakuMidnightCalendarInstant(value: DateInput): boolean {
  const zoned = dayjs(value).tz(APP_TIMEZONE);
...
export function toBaku(value: DateInput): Dayjs | null {
  ...
  // Respect Z / ± offsets (do not force utc() parse — that drops +04:00).
...
export function formatDateTime(value: DateInput): string {
  ...
  if (isUtcMidnightCalendarInstant(value)) {
    return dayjs.utc(value).format(DATETIME_DISPLAY_FORMAT);
  }
  if (isBakuMidnightCalendarInstant(value)) {
    return dayjs.utc(value).tz(APP_TIMEZONE).format(DATETIME_DISPLAY_FORMAT);
  }
...
export function formatDate(value: DateInput): string {
  ...
  if (isUtcMidnightCalendarInstant(value)) {
    return dayjs.utc(value).format(DATE_DISPLAY_FORMAT);
  }
  if (isBakuMidnightCalendarInstant(value)) {
    return dayjs.utc(value).tz(APP_TIMEZONE).format(DATE_DISPLAY_FORMAT);
  }
  const zoned = toBaku(value);
  if (!zoned) return '—';
...
export function toDateOnlyApi(value: DateInput): string | null {
  ...
  if (isUtcMidnightCalendarInstant(value)) {
    return dayjs.utc(value).format(DATE_ONLY_API_FORMAT);
  }
  const zoned = toBaku(value);
...
/**
 * Cash create/update: calendar `YYYY-MM-DD` + current Asia/Baku clock → one instant.
 * Date picker only sends a day; without this, lists show a misleading `00:00`.
 */
export function coerceCashTransactionDateTime(
  value: unknown,
  ...
  if (typeof value === 'string') {
    const trimmed = value.trim();
    if (isDateOnlyApiString(trimmed)) {
      const clock = dayjs(now).tz(APP_TIMEZONE);
      const combined = dayjs.tz(
        `${trimmed} ${clock.format('HH:mm:ss.SSS')}`,
        'YYYY-MM-DD HH:mm:ss.SSS',
        APP_TIMEZONE,
      );
      return combined.isValid() ? combined.toDate() : undefined;
    }
    const parsed = dayjs(trimmed);
    return parsed.isValid() ? parsed.toDate() : undefined;
  }
...
/**
 * Serialize an instant for JSON API responses in Asia/Baku with numeric offset.
 * Example: `2026-08-02T00:10:35.347+04:00` (never bare `...Z` for UI consumers).
 * Calendar date-only values (UTC/Baku midnight) serialize as that day at 00:00+04:00.
 */
export function toApiDateTime(value: Date | string): string {
  if (
    isUtcMidnightCalendarInstant(value) ||
    isBakuMidnightCalendarInstant(value)
  ) {
    const dateOnly = toDateOnlyApi(value);
    if (!dateOnly) {
      return dayjs(value).tz(APP_TIMEZONE).format('YYYY-MM-DDTHH:mm:ss.SSSZ');
    }
    return dayjs
      .tz(dateOnly, DATE_ONLY_API_FORMAT, APP_TIMEZONE)
      .format('YYYY-MM-DDTHH:mm:ss.SSSZ');
  }
  return dayjs(value).tz(APP_TIMEZONE).format('YYYY-MM-DDTHH:mm:ss.SSSZ');
}

export function toApiDateTimeOrNull(
