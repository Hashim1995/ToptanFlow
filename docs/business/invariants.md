...
- **[Approved Human Decision — recorded 2026-08-03; ADR-040 / CHANGE-019.]** The same narrow Super Admin boundary also governs Cash Account creation and responsible-user assignment/change. **[Amended CHANGE-028, 2026-08-04.]** Super Admin may also correct an existing Cash Account opening balance. It does not restrict Cash operations: every active user may operate any active Cash Account, and the authenticated operator remains the recorded actor.
...
- **Opening balance** is posted as an auditable `OPENING_BALANCE` movement when an account starts with a non-zero balance. Later corrections must not silently rewrite posted amounts (ADR-033). **[Approved Human Decision — CHANGE-028, recorded 2026-08-04.]** Only a Super Admin may correct an existing Cash Account’s opening balance: cancel the active `OPENING_BALANCE` via immutable reversal when present, then post a new `OPENING_BALANCE` when the corrected amount is greater than zero. Ordinary cash history is not rebuilt; `currentBalance` remains the sum of signed movement effects.
